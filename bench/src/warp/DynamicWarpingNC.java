...
public class DynamicWarpingNC {
   ...
   */
  public DynamicWarpingNC(int shiftMin, int shiftMax) {
   ...
   *  of shifts.
   */
  public DynamicWarpingNC(int shiftMin, int shiftMax, int fr) {
  ...
  public void setKMax(int kmax) {
    setKMax(kmax,kmax,kmax);
  }
  
  public void setKMax(int kmax1, int kmax2) {
    setKMax(kmax1,kmax2,kmax2);
  }
  
  public void setKMax(int kmax1, int kmax2, int kmax3) {
    _kmax1 = kmax1;
    _kmax2 = kmax2;
    _kmax3 = kmax3;
  }
  ...
  public void backtrackReverse2(float[][][] d, float[][][] e, float[][] u) {
    ...
    for (int i1=0; i1<n1; ++i1) {
      ...
      backtrack(-1,_kmax2,_shifts,di1,ei1,ui1);
      for (int i2=0; i2<n2; ++i2)
        u[i2][i1] = ui1[i2];
    }
  }
...
//  private int _bstrain3 = 1; // inverse of bound on strain in 3rd dimension
  private int _kmax1 = 1; // maximum stencil distance in first dimension
  private int _kmax2 = 1; // maximum stencil distance in second dimension
  private int _kmax3 = 1; // maximum stencil distance in third dimension
   ...
   * @param kmax maximum stencil distance.
   * @param e input array[ni][nl] of alignment errors.
   * @param d output array[ni][nl] of accumulated errors.
   */
  private static void accumulate(int dir, int kmax, float[][] e, float[][] d) {
    int nl = e[0].length;
    int ni = e.length;
    int nlm1 = nl-1;
    int nim1 = ni-1;
  ...
  private static void backtrack(
    int dir, int kmax, Sampling shifts, float[][] d, float[][] e, float[] u) 
  {
    int nl = d[0].length;
    int ni = d.length;
    int nlm1 = nl-1;
...
//    }
   ...
   * @param kmax maximum stencil distance in 1st dimension.
   * @param e input array of alignment errors to be smooothed.
   * @param es output array of smoothed alignment errors.
   */
  private static void smoothErrors1(int kmax, float[][] e, float[][] es) {
    int nl = e[0].length;
    int n1 = e.length;
    float[][] ef = new float[n1][nl];
    float[][] er = new float[n1][nl];
    accumulate( 1,kmax,e,ef);
    accumulate(-1,kmax,e,er);
    for (int i1=0; i1<n1; ++i1)
      for (int il=0; il<nl; ++il)
  ...
  private void smoothErrors(float[][][][] e) {
    ...
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      smoothErrors1(_kmax1,ef[i3],ef[i3]);
    }});
    normalizeErrors(e);
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      smoothErrors2(_kmax2,ef[i3],ef[i3]);
    }});
    normalizeErrors(e);
    Parallel.loop(n2,new Parallel.LoopInt() {
    public void compute(int i2) {
